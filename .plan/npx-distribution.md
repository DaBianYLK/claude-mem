...
2. **Create `src/npx-cli/index.ts`** — a Node.js CLI router (NOT Bun) with command categories:
   ...
   **Runtime commands must check for installation first**: If plugin directory doesn't exist at `~/.claude/plugins/marketplaces/DaBianYLK/`, print "claude-mem is not installed. Run: npx claude-mem install" and exit.

3. **The install flow** (fully replaces git clone + build):
   - Detect the npm package's own location (`import.meta.url` or `__dirname`)
   - Copy `plugin/` from the npm package to `~/.claude/plugins/marketplaces/DaBianYLK/`
   - Copy `plugin/` to `~/.claude/plugins/cache/DaBianYLK/claude-mem/<version>/`
