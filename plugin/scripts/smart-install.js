...
function isPluginDisabledInClaudeSettings() {
  try {
    ...
    return settings?.enabledPlugins?.['claude-mem@DaBianYLK'] === false;
  } catch {
    return false;
  }
}
 ...
 * 3. XDG path (~/.config/claude/plugins/marketplaces/DaBianYLK)
 * 4. Legacy path (~/.claude/plugins/marketplaces/DaBianYLK)
 */
function resolveRoot() {
  ...
  const marketplaceRel = join('plugins', 'marketplaces', 'DaBianYLK');
