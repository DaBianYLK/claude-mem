...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:g,correlationId:h,...l}=n;Object.keys(l).length>0&&(u=` {${Object.entries(l).map(([f,R])=>`${f}=${R}`).join(", ")}}`)}let T=`[${i}] [${a}] [${p}] ${c}${s}${u}${m}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},_=new W;var Rt={};function ht(){return typeof __dirname<"u"?__dirname:(0,S.dirname)((0,de.fileURLToPath)(Rt.url))}var Ot=ht(),N=I.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,S.join)((0,ie.homedir)(),".claude"),Ht=(0,S.join)(y,"plugins","marketplaces","DaBianYLK"),Bt=(0,S.join)(N,"archives"),Wt=(0,S.join)(N,"logs"),Yt=(0,S.join)(N,"trash"),qt=(0,S.join)(N,"backups"),Vt=(0,S.join)(N,"modes"),Kt=(0,S.join)(N,"settings.json"),pe=(0,S.join)(N,"claude-mem.db"),Jt=(0,S.join)(N,"vector-db"),Qt=(0,S.join)(N,"observer-sessions"),zt=(0,S.join)(y,"settings.json"),Zt=(0,S.join)(y,"commands"),es=(0,S.join)(y,"CLAUDE.md");function ce(r){(0,ae.mkdirSync)(r,{recursive:!0})}function _e(){return(0,S.join)(Ot,"..")}var me=require("crypto");var Nt=3e4;function ue(r,e,t){return(0,me.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function le(r,e,t){let s=t-Nt;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=pe){e!==":memory:"&&ce(N),this.db=new Ee.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),p=new Date(a).toISOString(),c=ue(e,s.title,s.narrative),m=le(this.db,c,a);if(m)return{id:m.id,createdAtEpoch:m.created_at_epoch};let T=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,c,p,a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),p=new Date(a).toISOString(),m=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,p,a);return{id:Number(m.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let p=a??Date.now(),c=new Date(p).toISOString();return this.db.transaction(()=>{let u=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
      ...
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,c,p);u.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          ...
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,c,p);E=Number(h.lastInsertRowid)}return{observationIds:u,summaryId:E,createdAtEpoch:p}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,p=0,c){let m=c??Date.now(),u=new Date(m).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let b of s){let f=g.run(e,t,b.type,b.title,b.subtitle,JSON.stringify(b.facts),b.narrative,JSON.stringify(b.concepts),JSON.stringify(b.files_read),JSON.stringify(b.files_modified),a||null,p,u,m);E.push(Number(f.lastInsertRowid))}let h;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,p,u,m);h=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(m,o),{observationIds:E,summaryId:h,createdAtEpoch:m}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),c=[...e],m=o?`WHERE id IN (${p}) AND project = ?`:`WHERE id IN (${p})`;return o&&c.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${m}
      ORDER BY created_at_epoch ${i}
      ${a}
    `).all(...c)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),c=[...e],m=o?"AND s.project = ?":"";return o&&c.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
...
`}function et(r,e,t,s){let n=[];return s?n.push(...je(r)):n.push(...Ie(r)),s?n.push(...Ge()):n.push(...ye()),s?n.push(...He()):n.push(...Le()),s?n.push(...Be()):n.push(...ve()),P(t)&&(s?n.push(...We(e,t)):n.push(...Me(e,t))),n}var ee=v(require("path"),1);function H(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return _.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function st(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function rt(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function tt(r,e){return ee.default.isAbsolute(r)?ee.default.relative(e,r):r}function ot(r,e,t){let s=H(r);if(s.length>0)return tt(s[0],e);if(t){let n=H(t);if(n.length>0)return tt(n[0],e)}return"General"}function yt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=nt(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function Lt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?H(r.facts).join(`
`):null}function vt(r,e,t,s,n,o){let i=[];o?i.push(...Ye(r)):i.push(...De(r));let a=null,p="",c=!1;for(let m of e)if(m.type==="summary"){c&&(i.push(""),c=!1,a=null,p="");let u=m.data,T=st(u.displayTime);o?i.push(...Je(u,T)):i.push(...we(u,T))}else{let u=m.data,T=ot(u.files_modified,n,u.files_read),E=rt(u.created_at),g=E!==p,h=g?E:"";p=E;let l=t.has(u.id);if(T!==a&&(c&&i.push(""),o?i.push(...qe(T)):i.push(...Ue(T)),a=T,c=!0),l){let b=Lt(u,s);o?i.push(...Ke(u,E,g,b,s)):(c&&!o&&(i.push(""),c=!1),i.push(...ke(u,h,b,s)),a=null)}else o?i.push(Ve(u,E,g,s)):i.push(xe(u,h,s))}return c&&i.push(""),i}function it(r,e,t,s,n){let o=[],i=yt(r);for(let[a,p]of i)o.push(...vt(a,p,e,t,s,n));return o}function at(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function dt(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,d.blue)),t.push(...x("Learned",r.learned,d.yellow)),t.push(...x("Completed",r.completed,d.green)),t.push(...x("Next Steps",r.next_steps,d.magenta))):(t.push(...U("Investigated",r.investigated)),t.push(...U("Learned",r.learned)),t.push(...U("Completed",r.completed)),t.push(...U("Next Steps",r.next_steps))),t}function pt(r,e){return e?Qe(r):$e(r)}function ct(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?ze(r.totalDiscoveryTokens,r.totalReadTokens):Fe(r.totalDiscoveryTokens,r.totalReadTokens)}var Mt=_t.default.join((0,mt.homedir)(),".claude","plugins","marketplaces","DaBianYLK","plugin",".install-version");function Dt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,ut.unlinkSync)(Mt)}catch(e){_.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return _.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ut(r,e){return e?Ze(r):Pe(r)}function xt(r,e,t,s,n,o,i){let a=[],p=K(e);a.push(...et(r,p,s,i));let c=t.slice(0,s.sessionCount),m=Ne(c,t),u=Z(e,m),T=Ce(e,s.fullObservationCount);a.push(...it(u,T,s,n,i));let E=t[0],g=e[0];at(s,E,g)&&a.push(...dt(E,i));let h=z(e,s,o,n);return a.push(...pt(h,i)),a.push(...ct(p,s,i)),a.join(`
`).trimEnd()}async function te(r,e=!1){let t=Y(),s=r?.cwd??process.cwd(),n=ge(s),o=r?.projects||[n],i=Dt();if(!i)return"";try{let a=o.length>1?Oe(i,o,t):J(i,n,t),p=o.length>1?Re(i,o,t):Q(i,n,t);return a.length===0&&p.length===0?Ut(n,e):xt(n,a,p,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
