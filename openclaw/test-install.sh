...
assert_contains "$CLAUDE_MEM_REPO" "github.com/DaBianYLK/claude-mem" "CLAUDE_MEM_REPO points to correct repository"
...
# Test: find_claude_mem_install_dir() — found in ~/.claude/plugins/marketplaces/DaBianYLK/
test_find_install_dir_marketplace() {
  ...
  mkdir -p "${fake_home}/.claude/plugins/marketplaces/DaBianYLK/plugin/scripts"
  touch "${fake_home}/.claude/plugins/marketplaces/DaBianYLK/plugin/scripts/worker-service.cjs"

  if find_claude_mem_install_dir 2>/dev/null; then
    test_pass "find_claude_mem_install_dir finds dir in marketplace path"
    assert_eq "${fake_home}/.claude/plugins/marketplaces/DaBianYLK" "$CLAUDE_MEM_INSTALL_DIR" "CLAUDE_MEM_INSTALL_DIR set correctly for marketplace"
  else
...
test_install_sh_has_stable_url_in_usage() {
  if grep -q 'raw.githubusercontent.com/DaBianYLK/claude-mem/main/openclaw/install.sh' "$INSTALL_SCRIPT"; then
