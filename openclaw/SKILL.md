...
git clone https://github.com/DaBianYLK/claude-mem.git
cd claude-mem
npm install
npm run build
```
...
If claude-mem is installed as a Claude Code plugin (at `~/.claude/plugins/marketplaces/DaBianYLK/`), start the worker from that installation:
...
| Worker started from Claude Code install but not responding | Check `cd ~/.claude/plugins/marketplaces/DaBianYLK && npm run worker:status`. May need `npm run worker:restart`. |
